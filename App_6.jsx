...
const TIERS = [
  ...
  },
  {
    ...
    features: [
      ...
      "Location-specific scavenger hunt — not findable anywhere else",
      "Dinner table questions every evening — no right answers",
      "Age-specific bedtime story the night before you land",
    ],
    cta: "Get Playbook — $27",
    highlight: true,
  },
  {
    ...
    features: [
      "Everything in Playbook — scavenger hunt, dinner questions & more",
      "Personal Kid Briefings — written directly to each child by name",
      "Top or Flop daily game — gets kids arguing, talking, engaged",
      "Museum Find-It mission — tailored to your child's exact interests",
      "Memory journal — one vivid detail per night, written to each child by name",
    ],
...
function buildPrompt(answers, tier) {
  ...
  // Parse number of days from duration string — take the highest number in the range
  const allNums = answers.duration?.match(/\d+/g);
  const numDays = allNums ? Math.max(...allNums.map(Number)) : 3;
  const daysToWrite = Math.min(numDays, 7);
...
## Memory Journal 📖
5 evening prompts for ${kidNames}. One per night. Each prompt asks for ONE specific detail — something they noticed, touched, smelled, or heard. Not a summary. Not a favourite. The one detail that caught their eye that nobody else in the family mentioned. Written directly to ${kidNames} by name. The kind they want to answer every night.

${(hasMid || hasTeen) ? `## Top or Flop 🏆
One Top or Flop prompt per day for ${kidNames}. A specific experience, meal, or sight from that day's itinerary. They must pick Top or Flop AND explain why. No right answer. Format: "Top or Flop: [specific thing from today]? You have to pick one — and explain your answer." Make it slightly controversial so they argue about it. Include one per day for exactly ${daysToWrite} days.` : ""}

## Museum Find-It 🔍
For any day that includes a museum, gallery, or historic site: write one Find-It mission tailored to ${kidNames}'s interests (${interests.length > 0 ? interests.join(", ") : "general curiosity"}). Give them one specific thing to hunt for across the whole space connected to what they already love. Format: "Your mission at [place]: find every [interest-connected thing] you can spot. Count them. Which one is your favourite and why?" End with a question that has no right answer. If no museum day exists in the itinerary, skip this section.` : ""}
...
## ✅ Trip Checklist — Print & Pack This

Write a clean printable checklist with these exact sections. Keep each item to one line. This is meant to be printed and carried.

**🔍 Our Trip Mystery**
- One sentence stating the mystery to solve

**✨ One Moment Per Day — Phones Away**
${Array.from({length: daysToWrite}, (_, i) => `- Day ${i+1}: [specific moment]`).join('\n')}

**🎯 Daily Missions**
${Array.from({length: daysToWrite}, (_, i) => `- Day ${i+1}: [one specific thing to find or do]`).join('\n')}

${tier.id !== "essential" ? `**🔍 Scavenger Hunt Items**
- [ ] [item 1]
- [ ] [item 2]
- [ ] [item 3]
- [ ] [item 4]
...
**🧠 Sticky Facts to Share**
${Array.from({length: daysToWrite}, (_, i) => `- Day ${i+1}: [sticky fact]`).join('\n')}

**📍 Key Places**
${Array.from({length: daysToWrite}, (_, i) => `- Day ${i+1} Morning: [place] | Afternoon: [place] | Evening: [place]`).join('\n')}`;
...
function renderPlaybook(text, answers, tier) {
  ...
  const rendered = lines.map((line, i) => {
    ...
    // Convert --- to a visual divider
    if (/^---+$/.test(line.trim())) return (
      <div key={i} style={{ height: 1, background: "#E1F5FE", margin: "1rem 0" }} />
    );
    ...
    // Checkbox items — render as printable checkboxes
    if (/^-\s\[\s\]/.test(line)) return (
      <div key={i} style={{ display: "flex", gap: "0.7rem", alignItems: "center", marginBottom: "0.5rem", paddingLeft: "0.5rem" }}>
        <div style={{ width: 18, height: 18, border: "2px solid #0288D1", borderRadius: 4, flexShrink: 0, background: "#fff" }} />
        <p style={{ fontSize: "0.9rem", color: "#334", lineHeight: 1.6, margin: 0 }} dangerouslySetInnerHTML={{ __html: md(line.replace(/^-\s\[\s\]\s/, "")) }} />
      </div>
    );
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/*" element={<LittleRoutesApp />} />
      </Routes>
    </BrowserRouter>
  );
}

function LittleRoutesApp() {
    ...
    if (params.get("success") === "true") {
      ...
      if (savedAnswers && savedTier && savedEmail) {
        ...
        .then(data => {
          ...
          setResult(data.result || "Could not generate.");
          setStep("result");
        })
        .catch(e => {
  ...
  async function handleCheckout() {
    ...
    const baseUrls = {
      essential: "https://littleroutesco.lemonsqueezy.com/checkout/buy/682aaad6-f258-4a45-bc2b-0d37966d42ba",
      playbook: "https://littleroutesco.lemonsqueezy.com/checkout/buy/d816b0a1-1f04-4a7a-8b7f-99386e51ba55",
      "family-pack": "https://littleroutesco.lemonsqueezy.com/checkout/buy/51229d97-fb45-440e-b6ff-afad6bb625ed",
    };

    console.log("Selected tier ID:", selectedTier.id);
    console.log("Going to URL:", baseUrls[selectedTier.id]);

    const baseUrl = baseUrls[selectedTier.id];
    const params = new URLSearchParams();
    params.set("checkout[email]", email);
    params.set("checkout[custom][redirect]", "https://littleroutes.co?success=true");
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media(max-width:768px) {
          ...
          .qcard { padding:1.5rem 1.2rem; border-radius:20px; }
          .q-head { font-size:1.5rem; }
          .multi-grid { grid-template-columns:1fr; }
          .opt { font-size:0.88rem; padding:0.8rem 1rem; }
          .tiers-section { padding:2rem 1rem 3rem; }
          .tiers-grid { grid-template-columns:1fr; }
          .tcard { padding:1.5rem 1.2rem; }
          .tprice { font-size:2rem; }
          .cocard { padding:1.5rem 1.2rem; border-radius:20px; }
          .result-outer { padding:0.5rem 0 4rem; }
          .result-banner { padding:1.5rem 1.2rem; border-radius:16px 16px 0 0; }
          .result-body { padding:1.5rem 1.2rem; border-radius:0 0 16px 16px; }
          .rb-title { font-size:1.6rem !important; }
          .rb-chips { gap:0.3rem; }
          .rb-chip { font-size:0.65rem; padding:0.2rem 0.5rem; }
          .btn-main { font-size:0.95rem; padding:0.9rem 1.4rem; width:100%; }
          .btn-main.yellow { width:auto; }
          .page { overflow-x:hidden; }
          h1 { font-size:clamp(2rem,8vw,3rem) !important; }
          /* Band padding on mobile */
          .band-pad { padding:2.5rem 1.2rem !important; }
        }
      `}</style>
      ...
      <nav>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1.2rem" }}>
          <a href="/blog" style={{ fontFamily: "'Fredoka', sans-serif", fontSize: "0.82rem", fontWeight: 700, color: "#0288D1", textDecoration: "none" }}>Blog</a>
      ...
      <main className="main">
        ...
        {step === "landing" && (
          <div className="page">
            ...
            <div className="band-pad" style={{ background:"#fff", padding:"4rem 2rem 3rem", position:"relative", overflow:"hidden" }}>
              ...
              <div className="hero-two-col" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", alignItems:"center", position:"relative", zIndex:1 }}>
                <div>
            ...
            <div style={{ background:"#fff", padding:"3.5rem 2rem 4rem" }}>
              <div style={{ maxWidth:960, margin:"0 auto" }}>
                ...
                <div className="preview-grid" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:"1.5rem" }}>
        ...
        {/* ── TIERS ── */}
        {step === "tiers" && (
          <div className="tiers-section page" id="pricing">
            ...
            <div className="tiers-grid">
              {TIERS.map(tier => (
                <div key={tier.id} id={tier.id} className={`tcard ${tier.highlight ? "pop" : ""}`}
            ...
            <div style={{ background:"#F3FAFF", padding:"2rem", textAlign:"center" }}>
              <div style={{ background:"#fff", border:"2px solid #E1F5FE", borderRadius:16, padding:"1.2rem 1.5rem", maxWidth:560, margin:"0 auto 1.2rem", boxShadow:"0 4px 0 #E1F5FE" }}>
                <p style={{ fontSize:"0.85rem", color:"#607D8B", fontWeight:600, marginBottom:"1rem" }}>
                  📬 Your family's adventure starts here — save your playbook below.
                </p>
                <button
      ...
      {/* FOOTER */}
      <footer style={{ background:"#1a3a6b", padding:"2rem", textAlign:"center", borderTop:"4px solid #C6FF00" }}>
        <div style={{ maxWidth:960, margin:"0 auto" }}>
          <p style={{ fontFamily:"'Fredoka',sans-serif", fontSize:"1rem", fontWeight:700, color:"#fff", marginBottom:"0.5rem" }}>
            🧭 Little<span style={{ color:"#0288D1" }}>Routes</span>
          </p>
          <p style={{ fontFamily:"'Nunito',sans-serif", fontSize:"0.8rem", color:"rgba(255,255,255,0.5)", marginBottom:"0.5rem" }}>
            Family Travel Playbooks · littleroutes.co
          </p>
          <p style={{ fontFamily:"'Nunito',sans-serif", fontSize:"0.8rem", color:"rgba(255,255,255,0.5)", marginBottom:"0.5rem" }}>
            Questions? Email us: <a href="mailto:littleroutes.co@gmail.com" style={{ color:"#C6FF00", textDecoration:"none", fontWeight:700 }}>littleroutes.co@gmail.com</a>
          </p>
          <p style={{ fontFamily:"'Nunito',sans-serif", fontSize:"0.75rem", color:"rgba(255,255,255,0.3)", marginTop:"1rem" }}>
            © 2026 LittleRoutes. All rights reserved.
          </p>
        </div>
      </footer>
    </>
