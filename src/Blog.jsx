        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1.5rem" }}>
          {sorted.map(post => (
            <Link key={post.slug} to={`/blog/${post.slug}`} style={{ textDecoration: "none" }}>
              <div className="post-card" style={{ background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.07)", border: "1.5px solid #E1F5FE", height: "100%", display: "flex", flexDirection: "column" }}>
                {/* Card header */}
                <div style={{ background: post.headerColor || "#1a3a6b", padding: "1.5rem 1.2rem 1.2rem" }}>
                  ...
                  <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: "1.15rem", fontWeight: 700, color: "#fff", lineHeight: 1.3, margin: 0, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: "4.5rem" }}>
                    {post.title}
                  </h2>
                </div>

                {/* Card body */}
                <div style={{ padding: "1rem 1.2rem 1.2rem", display: "flex", flexDirection: "column", justifyContent: "space-between", flex: 1 }}>
                  <p style={{ fontSize: "0.88rem", color: "#607D8B", lineHeight: 1.6, marginBottom: "1rem", display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: "5.6rem" }}>
                    {post.excerpt}
                  </p>
