...
function renderMarkdown(md) {
  ...
  return md
    // H2
    .replace(/^## (.+)$/gm, '<h2 style="font-family:\'Fraunces\',serif;font-size:1.5rem;font-weight:700;color:#1a3a6b;margin:2rem 0 0.8rem;line-height:1.2;">$1</h2>')
    // H3
    .replace(/^### (.+)$/gm, '<h3 style="font-family:\'Fredoka\',sans-serif;font-size:1.15rem;font-weight:700;color:#0288D1;margin:1.5rem 0 0.5rem;">$1</h3>')
    // Bold
    .replace(/\*\*(.+?)\*\*/g, '<strong style="font-weight:800;color:#1a3a6b;">$1</strong>')
    // Italic
    .replace(/\*(.+?)\*/g, '<em style="font-style:italic;color:#607D8B;">$1</em>')
    // Blockquote
    .replace(/^> (.+)$/gm, '<blockquote style="border-left:3px solid #C6FF00;background:#F3FAFF;margin:1.2rem 0;padding:0.8rem 1rem 0.8rem 1.2rem;border-radius:0 8px 8px 0;font-family:\'Georgia\',serif;font-style:italic;font-size:1rem;color:#1a3a6b;line-height:1.6;">$1</blockquote>')
    // Horizontal rule
    .replace(/^---$/gm, '<hr style="border:none;border-top:1px solid #E1F5FE;margin:2rem 0;"/>')
    // Unordered list items
    .replace(/^- (.+)$/gm, '<li style="margin-bottom:0.4rem;line-height:1.6;">$1</li>')
    // Wrap consecutive li in ul
    .replace(/(<li[^>]*>.*<\/li>\n?)+/g, (match) => `<ul style="padding-left:1.4rem;margin:0.8rem 0;color:#334;">${match}</ul>`)
    ...
    .map(block => {
      ...
      return `<p style="font-size:1rem;line-height:1.8;color:#334;margin-bottom:1rem;font-family:'Nunito',sans-serif;">${block.replace(/\n/g, " ")}</p>`;
    })
    .join("\n");
}
